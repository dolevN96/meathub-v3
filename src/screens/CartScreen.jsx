    ...
    <div className="fade-up">
      {cart.length === 0 ? (
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '80px 32px', textAlign: 'center' }}>
          <div style={{ width: 80, height: 80, borderRadius: '50%', background: 'rgba(255,255,255,.06)', display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: 20, border: '1px solid rgba(255,255,255,.1)' }}><Icon name="cart" size={36} color="#7A5F50" /></div>
          <div style={{ fontWeight: 800, fontSize: 18, marginBottom: 8, color: '#F5EDE4' }}>העגלה ריקה</div>
          <div style={{ fontSize: 14, color: '#7A5F50', marginBottom: 24 }}>הצטרף לקבוצת רכישה כדי להתחיל</div>
        ...
        <div className={isDesktop ? 'desktop-2col' : ''} style={{ gap: 24, alignItems: 'start' }}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }} className="px-mobile">
            {cart.map(item => {
              ...
              if (!p || !g) return null;
              return (
                <div key={item.groupId} style={{ background: '#1A100C', border: '1px solid rgba(240,192,96,.1)', borderRadius: 16, padding: 16, display: 'flex', gap: 14 }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ fontWeight: 700, fontSize: 15, marginBottom: 3, color: '#F5EDE4' }}>{p.name}</div>
                    <div style={{ fontSize: 12, color: '#7A5F50', marginBottom: 8, display: 'flex', alignItems: 'center', gap: 4 }}><Icon name="map" size={12} color="#7A5F50" />{g.location} · {g.pickup}</div>
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 8 }}>
                      <div>
                        <span style={{ fontWeight: 900, fontSize: 17, color: '#E8361A' }}>₪{(p.priceGroup * item.qty).toFixed(0)}</span>
                        <span style={{ fontSize: 12, color: '#7A5F50', marginRight: 6 }}>{item.qty} ק"ג × ₪{p.priceGroup}</span>
                      </div>
          ...
          <div style={{ position: isDesktop ? 'sticky' : 'static', top: 16 }} className="px-mobile">
            <div className="card">
              <div style={{ fontWeight: 800, fontSize: 15, marginBottom: 16, color: '#F5EDE4' }}>סיכום הזמנה</div>
              {[{ l: 'סכום ביניים', v: `₪${total.toFixed(0)}` }, { l: 'עמלת תפעול (6%)', v: `₪${(total * .06).toFixed(0)}` }].map(r => (
                <div key={r.l} style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 10 }}>
                  <span style={{ color: '#7A5F50', fontSize: 14 }}>{r.l}</span>
                  <span style={{ fontWeight: 600, fontSize: 14, color: '#C4A990' }}>{r.v}</span>
                </div>
              ))}
              <div style={{ height: 1, background: 'rgba(255,255,255,.08)', margin: '14px 0' }} />
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 16 }}>
                <span style={{ fontWeight: 800, fontSize: 16, color: '#F5EDE4' }}>סה"כ</span>
                <span style={{ fontWeight: 900, fontSize: 20, color: '#E8361A' }}>₪{(total * 1.06).toFixed(0)}</span>
              </div>
