...
## 🤖 תהליך הבנייה (Vibe Coding)

הפרויקט נבנה תוך עבודה צמודה עם Claude Code על בסיס שלד קוד קיים. השימוש המרכזי ב-AI היה בשלושה צירים:

- **דיבוג מקצה לקצה** — איתור באגים אמיתיים מול ה-DB החי (לדוגמה: שאילתת הזמנות שנכשלה כל הזמן כי ביקשה עמודות `color1`/`color2` שלא קיימות בסכמה האמיתית; trigger הרשמה ש"בלע" כל משתמש חדש; טוטאל ₪0 במסך אישור תשלום כי החישוב התבסס על עגלה שכבר נוקתה) — דרך בדיקה ישירה מול ה-API של Supabase (`curl`) כדי לאתר את הסיבה המדויקת ולא לנחש.
- **תכנון מודל נתונים** — תרגום הסכמה הקיימת ל-DBML/ERD מלא (טבלאות, טיפוסים, מפתחות, קשרים) להפקת תרשים ב-dbdiagram.io.
- **עבודת תשתית** — כתיבת פונקציית `pg_cron` שמחדשת אוטומטית את תאריכי הסיום של קבוצות פעילות, כך שהדמו החי נשאר רלוונטי לאורך זמן בלי תחזוקה ידנית; וניהול תהליך Git תקין (branch → PR → merge, ללא push ישיר ל-`master`).
